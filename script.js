...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        if (!allowedAdminEmails.includes(email.toLowerCase())) {
            loginError.textContent = 'Access Denied: Only authorized administrators can log in.';
        ...
        try {
            ...
            if (error.code === 'auth/user-not-found') {
                loginError.textContent = 'Admin not found. Please check your email.';
...
function createShipmentTableRow(data, withActions = false) {
    ...
    if (withActions) {
        ...
        const badgeClass = statusBadges[data.status] || 'status-created';
        statusCell.innerHTML = `<span class="status-badge ${badgeClass}">${data.status}</span>`;
...
async function createNewShipment() {
    try {
        ...
        const trackingNumber = 'SIC' + Math.random().toString(36).substr(2, 8).toUpperCase();
        const now = new Date();
